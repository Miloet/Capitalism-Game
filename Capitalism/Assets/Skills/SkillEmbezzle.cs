...
public class SkillEmbezzle : SkillBase
{
    public override void Start()
    {
        ...
        letter = "E";
        name = "Embezzle";
        description = $"Embezzle {FinancialDamage("(+10%)", true)} of the enemies money and gain it as your own causing them {GainStress("+1")} for every 1000$ of financial damage up to {GainStress("+3")}. " +
    ...
    public override void Effect(float multiplier = 1f)
    {
        ...
        float damage = currentAsset.value * 0.1f * multiplier * stressEffect;
        Enemy.money -= damage;
    ...
    public override string writeEffect()
    {
        ...
        if (currentAsset != null) effect = (currentAsset.value * 0.1f * GetMultiplier() * Mathf.Clamp(1 + Player.stress * 0.2f, 0, 2)).ToString("N0");
